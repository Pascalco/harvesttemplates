...
class Harvester extends React.Component {
  ...
  runHarvester(i) {
    let timeout = false;
    if (!(this.props.candidates[i])) { //done
      document.title = 'DONE - ' + document.title;
      this.setState({
        jobstatus: `done (${this.counts.success+this.counts.error}/${this.counts.total}, successes: ${this.counts.success}, errors: ${this.counts.error})`
      });
    ...
    .then(response => {
      ...
      if (timeout){
        this.setState({
          jobstatus: `waiting... Server lag (${this.counts.success+this.counts.error}/${this.counts.total}, successes: ${this.counts.success}, errors: ${this.counts.error})`
        });            
      } else {
        this.setState({
          jobstatus: `doing... (${this.counts.success+this.counts.error}/${this.counts.total}, successes: ${this.counts.success}, errors: ${this.counts.error})`
        });
      }
  ...
  handleStartStop(event) {
    ...
    if (this.run === true) {
      ...
      this.cancelToken = axios.CancelToken.source();
      this.setState({
        jobstatus: `stopped (${this.counts.success+this.counts.error}/${this.counts.total}, successes: ${this.counts.success}, errors: ${this.counts.error})`
      });
    } else if (this.Jwt){
      this.run = true;
      ...
      toast.error(<span id="toast">you are not logged in. <button onClick={this.onLogin} className="linkButton">login</button></span>, {position: toast.POSITION.TOP_CENTER});
    }
  }  
  ...
  createLink(){
    let permalink = '';
    let exportParameters = ['siteid', 'project', 'namespace', 'p', 'template', 'templateredirects', 'parameters', 'addprefix', 'removeprefix', 'addsuffix', 'removesuffix', 'searchvalue', 'replacevalue', 'category', 'depth', 'constraints', 'alreadyset', 'wikisyntax', 'manuallist'];
  ...
  render() {
    ...
    let columns = [{
      ...
      selector: 'title',
      sortable: true,
      format: row => <a href={`${this.props.job.site}/wiki/${row.title}`} target="_blank" rel="noopener noreferrer">{row.title}</a>
    }, {
      name: 'Wikidata item',
      selector: 'qid',
      sortable: true,
      format: row => <a href={`https://www.wikidata.org/wiki/${row.qid}`} target="_blank" rel="noopener noreferrer">{row.qid}</a>
    }, {
      name: 'raw value',
      selector: 'rawvalue',
      sortable: true
    ...
    <div className="fullwidth">
      <button onClick={this.handleStartStop} id="startButton" className='linkButton'>start</button><button onClick={this.handleDownload} className='linkButton'>download log</button><button onClick={this.handlePermalink} className='linkButton'>permalink</button><button onClick={this.handleSave} className='linkButton'>publicly save</button>
